...
export default class AudioRequests {
     ...
     */
    static GroupAudio = async (io: IO, id: number) => await io.Emit('group audio', { id });
     ...
     */
    static GroupAudioUpdate = async (io: IO, id: number, enabled: boolean, minRepLevel: number, stageId: number) => await io.Emit('group audio update', {
        headers: { version: 2 },
     ...
     */
    static GroupAudioSlotList = async (io: IO , id: number, subscribe: boolean) => await io.Emit('group audio slot list', { id, subscribe });
     ...
     */
    static GroupAudioSlotUpdate = async (io: IO, id: number, slotId: number, locked: boolean) => await io.Emit('group audio slot update', { id, slot: { id: slotId, locked } });
     ...
     */
    static GroupAudioBroadcast = async (io: IO, id: number, slotId: number, sdp: string) => await io.Emit('group audio broadcast', { id, slotId, sdp });
     ...
     */
    static GroupAudioBroadcastUpdate = async (io: IO, id: number, slotId: number, occupierId: number, occupierMuted: boolean) => await io.Emit('group audio broadcast update', {
     ...
     * @param occupierId the id of the subscriber making the request
     */
    static GroupAudioBroadcastDisconnect = async (io: IO, id: number, slotId: number, occupierId: number) => await io.Emit('group audio broadcast disconnect', {
    ...
    * @param sdp the session description protocol message
    */
    static GroupAudioConsume = async (io: IO, id: number, slotId: number, sdp: string) => await io.Emit('group audio consume', { id, slotId, sdp });
     ...
     */
    static StageList = async (io: IO, languageId?: number) => await io.Emit('stage list', (() => {
        let data: any = {};
     ...
     * @param id the id of the group
     */
    static StageGroupActiveList = async (io: IO, id: number) => await io.Emit('stage group active list', { id });
}