...
export default class IO {
     ...
     */
    constructor(client: Client, baseUrl: string = 'wss://v3.palringo.com', device: 'web' | 'android' | 'ios' = 'web') {
        this.Client = client;
     ...
     */
    On = (event: string, fn: Function): SocketIOClient.Emitter => this.Conn.on(event, fn);
     ...
     * @returns this emitter
     */
    Once = (event: string, fn: Function): SocketIOClient.Emitter => this.Conn.once(event, fn);
     ...
     * @returns this emitter
     */
    Off = (event: string, fn?: Function): SocketIOClient.Emitter => this.Conn.off(event, fn);
     ...
     */
    Emit = (event: string, data: any, batchSize: number = 50): Promise<any> => new Promise(async (resolve, reject) => {
        // Wrap in Body if just raw data is passed along
        if (!data.headers && !data.body)
            data = { body: data }
        
        if (!data.body.idList)
            this.Conn.emit(event, data, resp => resp.code >= 200 && resp.code <= 299 ? resolve(resp) : reject(resp));
        
        let idList: number[] = new Array<number>(...new Set<number>(data.body.idList));
        ...
        let req = (idList) => new Promise((resolve, reject) => {
            this.Conn.emit(event, {
                headers: data.headers ?? {},
                body: { ...data.body, idList }
            }, resp => resp.code >= 200 && resp.code <= 299 ? resolve(resp) : reject(resp));
        });

        let responses = (await asyncPool(3, chunks, req))
            .reduce((resps: any[], resp: any) => { resps.push(...Object.values(resp.body)); return resps; }, [])
            .reduce((entities: any[], entity: any) => { if (entity.code === 200) entities.push(entity.body); return entities; }, []);
